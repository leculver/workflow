...
├── config/
│   ├── repos.yaml                  #    Which repos you're triaging
│   └── local.yaml                  #    Local config: tools, repo paths, user identity (gitignored)
├── issues/
│   └── <owner>-<repo>/
│       └── <number>/
...
│   ├── specs/                      #    Design specs
│   └── study/                      #    Long-form technical documents (doc-loop)
...
Skills that need your GitHub username (e.g., filtering PRs by author) read from the `user.login` field in `config/local.yaml`. This file is auto-generated by the `bookkeeping` skill on first run via `gh api user` and is gitignored (machine-specific).
...
- `config/` — repo configuration (`repos.yaml`) and local-only machine config (`local.yaml`)
...
- `docs/notes/` — ad-hoc investigation notes
- `docs/specs/` — design specs
- `docs/study/` — long-form technical documents (generated by doc-loop)
