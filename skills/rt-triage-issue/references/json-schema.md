...
- `manually_investigated` is always `false` unless set by `rt-continue-issue`. Only that skill sets it to `true` — all other automated skills leave it as `false` or preserve the existing value.
